...
function R:EnsureUI()
    ...
    local f = addon.CreateThemedFrame and addon:CreateThemedFrame(UIParent, "HAK_MPlusRecruitment", 380, 440, 'panel') or CreateFrame("Frame", "HAK_MPlusRecruitment", UIParent, "BackdropTemplate")
    f:SetPoint("CENTER")
    if not f:GetWidth() or f:GetWidth() <= 0 then f:SetSize(380, 440) end
    local container
    if addon.ApplyStandardPanelChrome then
        container = addon:ApplyStandardPanelChrome(f, "Recruitment", { bodyPadding = { left = 18, right = 22, top = 78, bottom = 24 }, dragBody = true })
    end
    if not container then
        ...
        container:SetPoint("BOTTOMRIGHT", f, "BOTTOMRIGHT", -22, 24)
    end
    container = container or f
    -- Keystone display
    local keyLabel = container:CreateFontString(nil, "ARTWORK", "GameFontNormal")
    keyLabel:SetPoint("TOPLEFT", container, "TOPLEFT", 0, 0)
    keyLabel:SetText("Keystone:")
    local keyText = container:CreateFontString(nil, "ARTWORK", "GameFontHighlight")
    keyText:SetPoint("LEFT", keyLabel, "RIGHT", 8, 0)
    -- Spec selection (radio buttons)
    local specLabel = container:CreateFontString(nil, "ARTWORK", "GameFontNormal")
    ...
    local function MakeSpecButton(idx, text)
        local b = CreateFrame("CheckButton", "HAK_RecruitSpecBtn"..text, container, "UIRadioButtonTemplate")
    ...
    -- Needed roles
    local needLabel = container:CreateFontString(nil, "ARTWORK", "GameFontNormal")
    needLabel:SetPoint("TOPLEFT", specLabel, "BOTTOMLEFT", 0, -38)
    needLabel:SetText("Needed Roles:")
    local needTank = CreateFrame("CheckButton", "HAK_RecruitNeedTank", container, "InterfaceOptionsCheckButtonTemplate")
    needTank:SetPoint("TOPLEFT", needLabel, "BOTTOMLEFT", 0, -6)
    _G[needTank:GetName().."Text"]:SetText("Tank")
    needTank:SetHitRectInsets(2,2,2,2)
    local needHealer = CreateFrame("CheckButton", "HAK_RecruitNeedHealer", container, "InterfaceOptionsCheckButtonTemplate")
    needHealer:SetPoint("LEFT", needTank, "RIGHT", 80, 0)
    _G[needHealer:GetName().."Text"]:SetText("Healer")
    needHealer:SetHitRectInsets(2,2,2,2)
    local needDPS = CreateFrame("CheckButton", "HAK_RecruitNeedDPS", container, "InterfaceOptionsCheckButtonTemplate")
    ...
    local channelLabel = container:CreateFontString(nil, "ARTWORK", "GameFontNormal")
    channelLabel:SetPoint("TOPLEFT", needTank, "BOTTOMLEFT", 0, -20)
    channelLabel:SetText("Channel:")
    local channelDrop = CreateFrame("Frame", "HAK_RecruitChannelDropdown", container, "UIDropDownMenuTemplate")
    channelDrop:SetPoint("LEFT", channelLabel, "RIGHT", -10, -4)
    local previewLabel = container:CreateFontString(nil, "ARTWORK", "GameFontNormal")
    ...
    local previewBG = CreateFrame("Frame", nil, container, "BackdropTemplate")
    previewBG:SetPoint("TOPLEFT", previewLabel, "BOTTOMLEFT", -6, -2)
    previewBG:SetSize(320, 80)
    ...
    previewText:SetWidth(308)
    previewText:SetJustifyH("LEFT")
    -- Optional custom message suffix
    local customLabel = container:CreateFontString(nil, "ARTWORK", "GameFontNormal")
    customLabel:SetPoint("TOPLEFT", previewBG, "BOTTOMLEFT", 0, -8)
    customLabel:SetText("Custom Message:")
    local customBox = CreateFrame("EditBox", "HAK_RecruitCustomMsg", container, "InputBoxTemplate")
    ...
    local sendBtn = CreateFrame("Button", nil, container, "UIPanelButtonTemplate")
    sendBtn:SetSize(100,24)
    sendBtn:SetPoint("BOTTOMRIGHT", container, "BOTTOMRIGHT", 0, 0)
    sendBtn:SetText("Send")
    -- Full party timer display
    local fullTimerText = container:CreateFontString(nil, "ARTWORK", "GameFontNormal")
    fullTimerText:SetPoint("LEFT", sendBtn, "LEFT", -180, 0)
    fullTimerText:SetText("")
    if addon.StyleButton then addon:StyleButton(sendBtn) end
    if addon.StyleCheckbox then
        addon:StyleCheckbox(needTank)
        addon:StyleCheckbox(needHealer)
        addon:StyleCheckbox(needDPS)
    end
